 ...
 */
async function addData<T>(endpoint: URL, payload: Record<string, string>, displayFn: (t: T) => Record<string, string>): Promise<Errors.Result<Record<string, string>>> {
  try {
    const fetcheddata = await fetch(endpoint.toString(), {
      method: "PUT",
      headers: {
    ...
    const json_data = await fetcheddata.json();
    if (json_data.isOk) {
      return Errors.okResult(displayFn(json_data.result));
    } else {
      return new Errors.ErrResult(json_data.errors);
    }
  } catch (err) {
 ...
 */
async function findData<T>(endpoint: URL, displayFn: (t: T) => Record<string, string>): Promise<Errors.Result<PagedValues>> {
  try {
    const fetcheddata = await fetch(endpoint.toString(), {
      method: "GET",
      headers: {
    ...
    const json_data = await fetcheddata.json();
    if (json_data.isOk) {
      // console.log("OK res: ", json_data);
      const values = [...json_data.result].map((e) => {
        return displayFn(e.result);
      });
      const next = json_data?.links?.next?.href;
      const prev = json_data?.links?.prev?.href;
